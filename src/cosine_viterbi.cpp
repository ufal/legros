...
void subword_cosine_similarities(
  ...
  for(size_t begin = 0; begin < word.size(); ++begin) {
    for(size_t end = begin + 1; end < word.size() + 1; ++end) {
      ...
      // if the substring is not in the vocabulary, skip it
      if(subwords.count(subword) == 0)
        continue;

      int subw_index = subwords.at(subword);
...
void subword_cosine_similarities(
  ...
  for(size_t begin = 0; begin < word.size(); ++begin) {
    ...
    if(word.find(candidate_subword) != std::string::npos) {

      Eigen::VectorXf subw_embedding = subword_embeddings.row(candidate_subword_index);
      float subw_norm = subw_embedding.norm();
      float dotprod = word_embedding.dot(subw_embedding);
      float sim = dotprod / (emb_norm * subw_norm);

      similarities.insert({candidate_subword_index, sim});
    }
  ...
  for(size_t i = 1; i < word.size() + 1; ++i) {
    ...
    for(size_t j = 0; j < i; ++j) {
      ...
      if(subwords.count(candidate) == 0) {
        ...
        int subw_index = subwords.at(candidate);
        candidate_similarity = similarities.at(subw_index);
      }
...
float viterbi_decode(
  ...
  subword_cosine_similarities(similarities, word, word_embedding, subwords,
                              subword_embeddings,
                              candidate_subword, candidate_subword_index);
  ...
  for(size_t i = 1; i < word.size() + 1; ++i) {
    ...
    for(size_t j = 0; j < i; ++j) {
      ...
      if(subwords.count(candidate) == 0) {
        ...
        if(candidate == candidate_subword) {
          candidate_similarity = similarities.at(candidate_subword_index);
        }
        else if(j == i - 1) {
          candidate_similarity = -1;
        }
