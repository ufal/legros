...
void load_unigrams(unigram_table& unigram_frequencies,
...
float score_bigram(const std::string& subword,
  ...
  // fix to smooth
  return std::log((float)bigrams[prev][subword] / (float)unigrams[prev]);
}
...
void segment_token(std::vector<std::string>& segmentation,
  ...
  for(int row = 0; row < token.size(); ++row) {
    ...
    for(int col = row; col < max_column; ++col) {
      ...
      if(unigrams.count(subword) == 0)
        continue;

      if(row == 0) {
        float sc = score_bigram(subword, bow, unigrams, bigrams);
      ...
      for(int prev_row = min_prev_row; prev_row < row; ++prev_row) {
        ...
        if(unigrams.count(prev_subword) == 0)
          continue;

        if(score_table[prev_row][row - 1] == -std::numeric_limits<float>::infinity())
          continue;

        float bigram_score = score_bigram(subword, prev_subword, unigrams, bigrams)
...
int main(int argc, char* argv[]) {
  ...
  load_unigrams(unigram_frequencies, opt.unigram_stats);
